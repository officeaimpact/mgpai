"""
Test: Prefetch top-3 tours — 10 Russia domestic scenarios.
Each asks about 1st, 2nd or 3rd tour flight to verify cache hits.
"""
...
SCENARIOS = [
    {
        "name": "1. Сочи из Москвы",
        "search": "хочу в сочи из москвы, 15 июня на 7 ночей, 2 взрослых, 4 звезды завтрак",
        "flight_q": "какой перелёт в первом варианте?",
        "tour_pos": 1,
    },
    {
        "name": "2. Крым из Москвы",
        "search": "крым из москвы, начало августа на 7 ночей, 2 взрослых, 4 звезды все включено",
        "flight_q": "а какой перелёт во втором варианте?",
        "tour_pos": 2,
    },
    {
        "name": "3. Анапа из Москвы",
        "search": "анапа из москвы, 1 июля на неделю, 2 взрослых 1 ребёнок 5 лет, 4 звезды все включено",
        "flight_q": "расскажи про перелёт в третьем варианте",
        "tour_pos": 3,
    },
    {
        "name": "4. Калининград из Москвы",
        "search": "калининград из москвы, 20 мая на 5 ночей, 2 взрослых, 3 звезды завтрак",
        "flight_q": "какой перелёт в первом?",
        "tour_pos": 1,
    },
    {
        "name": "5. Мин.Воды из Москвы",
        "search": "минеральные воды из москвы, 10 июня на 7 ночей, 2 взрослых, 3 звезды завтрак",
        "flight_q": "какой рейс в третьем варианте?",
        "tour_pos": 3,
    },
    {
        "name": "6. Адлер из Казани",
        "search": "адлер из казани, конец июня на неделю, 2 взрослых, 4 звезды все включено",
        "flight_q": "какой перелёт в первом варианте?",
        "tour_pos": 1,
    },
    {
        "name": "7. Сочи из Екатеринбурга",
        "search": "сочи из екатеринбурга, 5 июля на 7 ночей, 2 взрослых, 4 звезды завтрак",
        "flight_q": "расскажи про перелёт во втором варианте",
        "tour_pos": 2,
    },
    {
        "name": "8. Абхазия из Москвы",
        "search": "абхазия из москвы, середина июля на неделю, 2 взрослых, 3 звезды все включено",
        "flight_q": "какой перелёт в первом?",
        "tour_pos": 1,
    },
    {
        "name": "9. Геленджик из Москвы",
        "search": "геленджик из москвы, июль на 7 ночей, 2 взрослых, 4 звезды",
        "flight_q": "какой перелёт в третьем?",
        "tour_pos": 3,
    },
    {
        "name": "10. Крым из Москвы (лето)",
        "search": "крым из москвы, август на 7 ночей, 2 взрослых, 4 звезды завтрак",
        "flight_q": "какой перелёт в первом варианте?",
        "tour_pos": 1,
    },
]
...
print("=" * 70, flush=True)
print("  ТЕСТ: PREFETCH 3 ТУРА — РОССИЯ (10 сценариев)", flush=True)
print("=" * 70, flush=True)
...
for sc in SCENARIOS:
    ...
    has_flight = any(w in r_f["reply"].lower() for w in [
        ...
        "аэрофлот", "s7", "уральские", "nord wind", "smartavia"
    ])
...
print(f"\n{'='*70}", flush=True)
print(f"  РЕЗУЛЬТАТЫ — РОССИЯ (prefetch 3)", flush=True)
